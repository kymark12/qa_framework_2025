...
def test_get_user(settings):
    ...
    response = requests.get(f"{settings.api.base_url}/users/1")
...
def test_create_user():
    ...
    new_user = {"name": "QA Test User", "username": "qa_test", "email": "qa@example.com"}
...
def test_update_user():
    ...
    response = requests.put("https://jsonplaceholder.typicode.com/users/1", json=updated_data)
...
def test_create_user_with_missing_fields():
    ...
    response = requests.post("https://jsonplaceholder.typicode.com/users", json=incomplete_user)
